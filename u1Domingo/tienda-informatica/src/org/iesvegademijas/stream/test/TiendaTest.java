...
class TiendaTest {
	...
	void test1() {
		...
		try {
			...
			//TODO STREAMS
			List<String> listaNomPrec = listProd.stream()
					.map(p -> "Nombre: " + p.getNombre() + ", Precio: " + p.getPrecio())
					.collect(toList());
			
			listaNomPrec.forEach(System.out::println);
	...
	void test2() {
		...
		try {
			...
			System.out.println("Listado productos precio euros:");
			listProd.forEach(System.out::println);
			
			//TODO STREAMS
			listProd.stream()
					.map(p -> {p.setPrecio(p.getPrecio() * 0.99); return p;}).collect(toList());
	...
	void test3() {
		...
		try {
			...
			//TODO STREAMS
			var listStr = listProd.stream()
					.map(p -> "Nombre: " + p.getNombre().toUpperCase()
											+ "Precio: " + p.getPrecio());
	...
	void test4() {
		...
		try {
			...
			//TODO STREAMS
			var list = listFab.stream().map(f -> f.getNombre() + "," + f.getNombre().substring(0, 2).toUpperCase());
	...
	void test6() {
		...
		try {
			...
			//TODO STREAMS
			var listaFabric = listFab.stream()
									.map(f -> f.getNombre())
									.sorted(reverseOrder());
			
			listaFabric.forEach(System.out::println);
	...
	void test7() {
		...
		try {
			...
			//TODO STREAMS
			var listaOrder = listProd.stream()
											.sorted((o1,o2) -> Double.compare(o2.getPrecio(), o1.getPrecio()))
											.sorted((o1,o2) -> o1.getNombre().compareToIgnoreCase(o2.getNombre()))
											// .sorted(comparing(Producto::getNombre).reversed().thenComparing(Producto::getPrecio))
											.map(Producto::getNombre)
											.collect(toList());
			
			listaOrder.forEach(System.out::println);
	...
	void test9() {
		...
		try {
			...
			//TODO STREAMS
			var listaReducida = listFab.stream()
										.skip(3)
										.limit(2)
										.collect(toList());
			
			listaReducida.forEach(System.out::println);
	...
	void test10() {
		...
		try {
			...
			//TODO STREAMS
			Optional<Producto> lista = listProd.stream()
	...
	void test17() {
		...
		try {
			...
			listProd.stream()
							.filter(p -> set.contains(p.getFabricante().getCodigo())).collect(toList());
			
			listProd.stream()
						.filter(p -> set.stream()
											.anyMatch(i -> p.getFabricante().getCodigo().equals(i)))
											.collect(toList());
	...
	void test23() {
		...
		try {
			...
			//TODO STREAMS
			var lista = listProd.stream()
						.sorted((o1,o2) -> o1.getFabricante().getNombre()
														.compareToIgnoreCase(o2.getFabricante().getNombre()))
						.map(p -> "Nombre: " + p.getNombre() + ", " + p.getPrecio())
						.collect(toList());
